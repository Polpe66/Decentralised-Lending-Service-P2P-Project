#!/usr/bin/env python3
# yesman: guarda i nuovi eventi ProposalSubmitted e vota sempre "approve" se non ha già votato e se è un contributor
...
YES_MAN_DEPOSIT_ETH = float(os.environ.get("YES_MAN_DEPOSIT", "0.1"))
LOG_FILE = os.environ.get("YES_MAN_LOG_FILE", str(DATA_DIR / "yes_man.log"))
...
def load_inputs(): # carica i dati di configurazione da accounts.json e lending_pool_info.json; verifica che i file esistano e che contengano le informazioni necessarie per lo yes_man
    ...
    ym = accounts.get("yes_man")
    if not ym or "key" not in ym:
        sys.exit("ERROR: yes_man entry missing in accounts.json")
    return ym, pool_info
...
def ensure_contributor(w3: Web3, pool, ym_addr: str, ym_key: bytes) -> bool: # verifica se yes_man è contributor
    if pool.functions.isContributor(ym_addr).call():
        log("INFO", "bootstrap", "isContributor=true", "skip deposit")
        return True

    amount_wei = Web3.to_wei(YES_MAN_DEPOSIT_ETH, "ether") # converte l'importo da ETH a Wei
    log("INFO", "bootstrap", f"deposit {YES_MAN_DEPOSIT_ETH} ETH", "isContributor=false, depositing",)
    nonce = w3.eth.get_transaction_count(ym_addr) # ottiene il nonce corrente per l'indirizzo dello yes_man, necessario per costruire la transazione
    tx = pool.functions.deposit().build_transaction({"from": ym_addr, "value": amount_wei, "nonce": nonce, "gas": 200_000, "gasPrice": w3.eth.gas_price, "chainId": w3.eth.chain_id,})
    rcpt = send_tx(w3, ym_key, tx) # invia la transazione di deposito e attende
...
def vote_approve(w3: Web3, pool, ym_addr: str, ym_key: bytes, proposal_id: int) -> None:
    try:
    ...
    if status != PROPOSAL_STATUS_ACTIVE:
        ...
        return
    if pool.functions.hasVotedOn(proposal_id, ym_addr).call(): # verifica se lo yes_man ha già votato sulla proposta tramite la funzione hasVotedOn del contratto
        log("INFO", "vote", f"pid={proposal_id}", "skipped - already voted")
        return
    if not pool.functions.isContributor(ym_addr).call(): # verifica se lo yes_man è un contributor tramite la funzione isContributor del contratto
        log("WARN", "vote", f"pid={proposal_id}", "skipped - yes_man is not a contributor",)
        return

    nonce = w3.eth.get_transaction_count(ym_addr) # ottiene il nonce corrente per l'indirizzo dello yes_man, necessario per costruire la transazione di voto
    tx = pool.functions.vote(proposal_id, True).build_transaction({"from": ym_addr, "nonce": nonce, "gas": 150_000, "gasPrice": w3.eth.gas_price, "chainId": w3.eth.chain_id,})
    try:
        rcpt = send_tx(w3, ym_key, tx) # ym_key è la chiave privata dello yes_man, necessaria per firmare la transazione; invia la transazione di voto e attende il receipt
    except Web3RPCError as e:
...
def main() -> int:
    ym, pool_info = load_inputs() # carica i dati di configurazione e verifica che siano presenti le informazioni necessarie per lo yes_man
    ym_addr = Web3.to_checksum_address(ym["address"]) # converte l'indirizzo dello yes_man in formato checksum (con lettere maiuscole e minuscole per evitare errori di digitazione)
    ym_key = bytes.fromhex(ym["key"][2:] if ym["key"].startswith("0x") else ym["key"]) # converte la chiave privata dello yes_man da stringa esadecimale a bytes
    ...
    log("INFO", "config", f"yes_man={ym_addr} pool={proxy_addr}", f"poll_interval={POLL_INTERVAL}s",)

    if not ensure_contributor(w3, pool, ym_addr, ym_key): # verifica se lo yes_man è già un contributor
        return 1
        ...
        try:
            ...
            if head >= last_block:
                ...
                for ev in events: # per ogni evento ProposalSubmitted, estraiamo i dettagli della proposta (pid, applicant, amount)
                    ...
                    vote_approve(w3, pool, ym_addr, ym_key, pid)
                last_block = head + 1
            rpc_failures = 0
        except Web3RPCError as e:
            rpc_failures += 1
